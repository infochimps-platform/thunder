    ...
    class Sherpa < Thor
      ...
      no_commands do
        def srpgo(command, args)
          ...
          #  1) compiling all of the client and foundation.rb templates to json
          #  2) uploading template jsons to unique-named folder in an s3 bucket
          ...
          # API access to s3 buckets can be a pain. For this one, we HAVE to go
          # through us-east-1. Even if you are going to be creating stacks in a
          # different region do not change this value unless you have set up 
          # your own bucket and you know what you are doing.
          bucket_region = "us-east-1"

          # Where to put generated files
          temp_dir = ".upload"

          # Name of the paramteter to use to tell the builder stack where to 
          # find its client jsons
          ...
          # If any generation parameters are supplied, they must be at the end 
          # of the thunder command line. These will be passed to cfndsl when we 
          # build the clients. Currently these must be yaml

          #Divide the parameters into generation and non-generation, 
          # pivoting around -g
